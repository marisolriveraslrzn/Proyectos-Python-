...
def update_list():
    if todolist.size() > 0:
        todolist.delete(0, "fin")
    for task in tasks:
        todolist.insert("fin", "[" + task[0] + "] Tiempo restante: " + str(task[1]) + " segundos")


def time_passed(task):
    tkinter.messagebox.showinfo("Notificación", "Es hora de: " + task)
...
if __name__ == '__main__':
    ...
    # widgets
    label = tkinter.Label(app, text="Trabajo a realizar:",
                          wraplength = 200,
                          justify = tkinter.LEFT)
    label_hour = tkinter.Label(app, text="Tiempo (segundos)",
                               wraplength = 200,
                               justify = tkinter.LEFT)
    todo = tkinter.Entry(app, width=30)
    time = tkinter.Entry(app, width=15)
    send = tkinter.Button(app, text='Añadir', fg="#ffffff", bg='#6186AC', height=3, width=30, command=get_entry)
    quit = tkinter.Button(app, text='Salir', fg="#ffffff", bg='#EB6464', height=3, width=30, command=app.destroy)
