...
class Specialization:
    ...
    def add_new_patient(self, name, status):
        if len(self.queue) >= self.MAX_CAPACITY:
            print("Disculpas, la cola está llena para esta especialización.")
            return
        if status not in self.PATIENT_STATUS_NUMBERS:
            print("Estado no válido. Debe ser 0 (normal), 1 (urgente) o 2 (superurgente).")
            return
        new_pat = Patient(name, status)
        self.queue.append(new_pat)
        self.queue.sort(key=lambda x: x.status, reverse=True)
        print(f"Paciente: {new_pat.name} es {self.format_patient_status(new_pat.status)}")
    ...
    def get_next_patient(self):
        ...
        print(f"{next_patient.name}, Por favor, vaya con el doctor.")
    ...
    def print_patients(self):
        ...
        for patient in patients:
            print(f"Paciente: {patient.name} es {self.format_patient_status(patient.status)}")

    def is_full(self):
        return len(self.queue) >= self.MAX_CAPACITY

    def __str__(self):
        return f"{self.name}: Hay {len(self.queue)} pacientes"
